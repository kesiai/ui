...
`FormRelate` 是关联字段的纯组件版本，不依赖表单上下文（`useFormContext`）或表格上下文（`Table2Context`）。所有需要的数据都通过 props 传入，使其可以在任何场景下独立使用。

- **纯组件设计**：不依赖外部上下文，可独立使用
...
| 特性 | FormRelate（纯组件） | FormRelateOld（旧版） |
|------|---------------------|---------------------|
...
1. **纯组件特性**：不使用任何 React Context，可以在任何地方使用。
