...
import FilterNumberRewrite from './fieldComponent/FilterNumberRewrite'
import FilterInputSelect from './fieldComponent/FilterInputSelect'
import ReferenceComponent from './fieldComponent/ReferenceComponent'
import FormulaComponent from './fieldComponent/FormulaComponent'
import FormInfoComponent from './fieldComponent/FormInfoComponent'
import TblnameComponent from './fieldComponent/TblnameComponent'
import FilterDate from './fieldComponent/FilterDate'
import LanguageType from './component/Editor/component/LanguageType'
import LanguageAndRegexInput from './component/Editor/component/LanguageAndRegexInput'
import WidgetsSelect from './component/Editor/component/WidgetsSelect'
import TableFieldRender from './TableFieldRender'

import { FilterUserRole } from './fieldComponent/UserRoleComponent'
import { FieldRules } from './component/FieldRules'
import { FormLayout, FieldGroup } from './component/FieldRules/components'
import { TCProvider } from './component/Editor/context'

const SchemaEditor = lazy(() => import('./component/Editor/SchemaEditor').then(m => ({ default: m.SchemaEditor2 })))

import { RelateTable, tableSchema, tableDataSchema } from './component/Editor/schema/relate'
import { registerWidgetSchema } from './component/Editor/component/Registry'
import TableSchema from './component/Editor/schema/table'
import { SingleUpload, MultipleUpload } from './component/Editor/schema/upload'
import RelateComponentPlus from './fieldComponent/RelateComponentPlus'

registerWidgetSchema('RelateTable', RelateTable);
registerWidgetSchema('tableSchema', tableSchema);
registerWidgetSchema('tableDataSchema', tableDataSchema);
registerWidgetSchema('TableSchema', TableSchema);
registerWidgetSchema('SingleUpload', SingleUpload);
registerWidgetSchema('MultipleUpload', MultipleUpload);
...
const custom = {
  ...
  form_fields: {
    ...
    relate_plus: { component: RelateComponentPlus }
  },
  schema_converter: [(f, schema, options) => {
    ...
    if (schema.enum1 && schema.enum1.length > 0) {
      ...
      if (schema.maxItems || schema.minItems) {
        f.validate = (val) => {
          if (val && val.length < schema.minItems) {
            return _t1('不应少于{{num}}个项', { num: schema.minItems })
          }
          if (val && val.length > schema.maxItems) {
            return _t1('不应多于{{num}}个项', { num: schema.maxItems })
          }
        }
      }
    }
    ...
    if (schema.config == '关联字段' && !schema.relateTo && !schema.relate) { // 关联字段未选择关联表时，走这个逻辑，不然走默认的Object组件，白屏
