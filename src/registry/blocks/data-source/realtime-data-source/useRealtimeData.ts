...
const sortByTags = (
  ...
  tags.forEach((tag, index) => {
    ...
    if (!tableData?.id || !tagValue?.tagId) {
      return
    }

    const key = buildTagKey(tagValue.tagId, tableData.id)
...
const findTagName = (tags: TagConfig[], key: string): string => {
  const tag = tags.find(t => {
    ...
    if (t.tableDataTag?.value) {
      const tagValue = t.tableDataTag.value
      const tagKey = buildTagKey(tagValue.tagId, tagValue.tableDataId)
      return tagKey === key
    }
...
const getTagItem = (tags: TagConfig[], key: string): TagConfig | undefined => {
  return tags.find(d => {
    ...
    if (d.tableDataTag?.value) {
      const tagValue = d.tableDataTag.value
      const tagKey = buildTagKey(tagValue.tagId, tagValue.tableDataId)
      return tagKey === key
    }
...
export function useRealtimeData(config: RealtimeDataConfig) {
  ...
  /**
   * 处理标签数据
   * 将 tableDataTag 格式转换为标准格式
   */
  const processTags = useCallback((inputTags: TagConfig[]) => {
    return (inputTags || [])
      .map(t => {
        // 如果有 tableDataTag，转换为标准格式
        if (t?.tableDataTag?.value) {
          const tagValue = t.tableDataTag.value
          return {
            ...t,
            table: t.table || { id: tagValue.tableId },
            tableData: t.tableData || {
              id: tagValue.tableDataId,
              name: t.tableDataTag.name,
              table: { id: tagValue.tableId }
            },
            tag: t.tag || { id: tagValue.tagId }
          } as TagConfig
        }
  ...
  const fetchHistory = useCallback(async () => {
    ...
    const where = subTags
      .map(s => {
        // 获取标签ID
        const tagId = s?.tag?.id || s?.tableDataTag?.value?.tagId

        // 获取表ID
        const tableId = s?.table?.id || s?.tableData?.table?.id || s?.tableDataTag?.value?.tableId

        // 获取表数据ID
        const tableDataId = s?.tableData?.id || s?.tableDataTag?.value?.tableDataId
    ...
    const subTagList = subTags.map(t => {
      ...
      if (t.tableDataTag?.value) {
        ...
        return {
          ...
          tableId: tagValue.tableId,
          dataId: tagValue.tableDataId
        }
      }
