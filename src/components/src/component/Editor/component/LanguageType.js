...
const LanguageType = ({ input, field, outStyle ,type }) => {
  const ifTable = field?.schema?.config === '文本' &&  field?.keyName !== 'defaultVal' // 是数据表中的文本控件，有自己的国际化配置
  ...
  const onLanageValue = (e, name) => {
    ...
    if (field.languageChange) {
      ...
      if (type === 'report') {
        const code = _.trim(key + '_' + name)
        form.change(`i18nProp.${code}`, value);
      } else {
  ...
  const hasLanguage = language?.length > 0 && app.module !== 'front'
  return (
    <div style={outStyle}>
      <div style={{ display: 'flex', borderRadius: '4px' }}>
        <InputNode
          ...
          onBlur={input?.onBlur}
          style={{ borderRadius: hasLanguage ? '4px 0px 0px 4px' : null }}
          {...field}
        />
        {
          hasLanguage && <Button className='language-btn' onClick={handleDropdownToggle} style={{
