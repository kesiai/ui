...
编辑表单中的 id 字段是特殊控件（`id-change-field` 组件）：默认只读 + 尾部 ✎ 按钮，点击经二次确认（危险提示）后解锁；输入新标识后点 ✓ **单独调用 change 接口**（`PATCH /core/t/{tableId}/d/change/{旧id}`，新 id 在 `body.id`）立即生效，不点则不保存；✕ 取消并恢复原标识。

- 修改标识等同删除重建：其它数据对它的引用（关联字段、时序数据、设备配置等）**不会自动迁移**，不可恢复
- 对话框底部的「保存」只管整条记录、与 id 无关，始终以当前生效 id 提交（标识改过之后自动用新 id）
...
```tsx
import { EditAction } from '@/components/kesi/view-actions/view-actions'
...
```tsx
import { DeleteAction } from '@/components/kesi/view-actions/view-actions'
...
```tsx
import { ExportAction } from '@/components/kesi/view-actions/view-actions'
