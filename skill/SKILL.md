...
6. ✅ 读取 `uiRouting.route`（UI 路由总开关）：
   - 若为 `self-loop` 或 `design-skill` → **短路退出**（本项目 UI 无需 kesi-ui），输出下方「短路输出模板」后结束，不执行任何步骤、不安装组件、不修改文件。
   - 若为 `kesi-ui` 或 `kesi-ui+design-skill` → 继续。
7. ✅ 若 `uiRouting.route = kesi-ui+design-skill`（组合模式）：仅处理 `uiRouting.crudPages` + `uiRouting.gisPages` 对应页面；其余展示型页面的视觉/布局交给用户指定的设计 skill（`uiRouting.designSkill`），kesi-ui 不越界。

任何验证失败 → 报告缺失项，不继续。

### 短路输出（当 `uiRouting.route` 不是 kesi-ui 时）

当 `uiRouting.route ∈ {self-loop, design-skill}`，输出以下友好说明后结束，**不空转**（零安装、零修改）：

> ℹ️ 本项目 UI 实现路由结果为 `uiRouting.route = <值>`，kesi-ui 本次跳过。
> - `self-loop`：kesi-frontend 已用 shadcn + echarts 自闭环完成全部展示型页面 UI，无需 kesi-ui。
> - `design-skill`：UI 视觉交由设计 skill `/<designSkill>` 实现，kesi-ui 不参与。
>
> 无组件安装、无代码修改。如需 kesi-ui 参与，请在 kesi-frontend 确认环节调整 UI 路由（增删 CRUD/GIS 页面或重新指定设计 skill）。
...
> ⚠️ **前置条件**：仅当 `design-report.json` 的 `uiRouting.route ∈ {kesi-ui, kesi-ui+design-skill}` 时本 skill 才执行；若为 `self-loop` / `design-skill`，已在输入验证阶段短路退出。
...
| uiRouting.route | kesi-ui 职责 |
|-----------------|------------|
| `kesi-ui`（独立） | 实现所有需专用组件的页面 + 其余页面 UI |
| `kesi-ui+design-skill`（组合） | **只做** CRUD（ViewModel 套件）和 GIS（GIS 套件）页面，见 `uiRouting.crudPages` / `gisPages`；页面视觉风格、布局、展示型组件交给设计 skill，不越界 |
